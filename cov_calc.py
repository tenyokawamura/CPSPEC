...
def main():
    ...
    kev_ref_min=ch2kev(ch=REF_MIN)
    kev_ref_max=ch2kev(ch=REF_MAX)
    n_b=len(COIS_MIN)
    kevs_bin=np.zeros(n_b)
    kevs_mea=np.zeros(n_b)
    kevs_sig=np.zeros(n_b)
    covs_per_mea=np.zeros(n_b)
    covs_per_sig=np.zeros(n_b)
    covs_abs_mea=np.zeros(n_b)
    covs_abs_sig=np.zeros(n_b)
    crs_mea_coi=np.zeros(n_b)
    crs_sig_coi=np.zeros(n_b)
    for i_b, (coi_min, coi_max) in enumerate(zip(COIS_MIN, COIS_MAX)):
        ...
        cr_sig_coi=cr_sig_calc(cr_mea=cr_mea_coi,\
                           t=exposure)
        crs_mea_coi[i_b]=cr_mea_coi
        crs_sig_coi[i_b]=cr_sig_coi
        ##### Read PSD (End) #####
        ...
        ##### Calculate covariance (Start) #####
        cov_per_mea,\
        cov_per_sig,\
        cov_abs_mea,\
        cov_abs_sig\
        =cov_calc_strict(fs=f_cut,\
                         psds_raw_ref=psd_raw_ref_cut,\
                         psds_noi_ref=psd_noi_ref_cut,\
                         cr_mea_ref=cr_mea_ref,\
        ...
        #          psds_noi_coi=psd_noi_coi_cut,\
        #          cr_mea_coi=cr_mea_coi,\
        #          csds_raw=csd_raw_cut,\
        #          ns_mer_i=n_mer_i_cut,\
        #          ns_mer_f=n_mer_f_cut)
        covs_per_mea[i_b]=cov_per_mea
        covs_per_sig[i_b]=cov_per_sig
        covs_abs_mea[i_b]=cov_abs_mea
        covs_abs_sig[i_b]=cov_abs_sig
        ##### Calculate covariance (End) #####

    ##### Write covariance (Start) #####
    write_cov(name_outtxt=name_outtxt,\
              cois_min=COIS_MIN,\
              cois_max=COIS_MAX,\
              f_min=F_MIN_RMS,\
              f_max=F_MAX_RMS,\
              covs_per_mea=covs_per_mea,\
              covs_per_sig=covs_per_sig,\
              covs_abs_mea=covs_abs_mea,\
              covs_abs_sig=covs_abs_sig,\
              crs_mea=crs_mea_coi,\
              crs_sig=crs_sig_coi,\
              exposure=exposure)
    ##### Write rms (End) #####

    ##### Plot covariance spectrum (Start) #####
    covs_abs_mea_kev=covs_abs_mea/kevs_bin
    covs_abs_sig_kev=covs_abs_sig/kevs_bin
    ...
    ax.set_ylabel('Fractional covariance (%)', fontsize=18)
    ...
    #          edgecolor='black',\
    #          fontsize=16)
    ax.set_title('f: {0:.1e} Hz - {1:.1e} Hz, Ref: {2:.1f} keV - {3:.1f} keV'\
                 .format(F_MIN_COV, F_MAX_COV, kev_ref_min, kev_ref_max), fontsize=18)
    #plt.show()
    out_pdf.savefig()
    plt.close()
    ...
    ax.set_yscale('log')
    ax.set_xlabel('Energy (keV)', fontsize=18)
    ax.set_ylabel('Covariance (s$^{-1}$ keV$^{-1}$)', fontsize=18)
    ax.tick_params(which='major',\
                   direction="in",\
                   length=10,\
                   width=1.0,\
