...
def main():
    ...
    for i_n, (ch_min, ch_max) in enumerate(zip(inputs_xspec.chs_min, inputs_xspec.chs_max)):
        ...
        # --------------------- #
        # ----- Read data ----- #
        name_infits=inputs_xspec.name_inevt.replace('.evt', '_{0:04}_{1:04}_psd.fits'.format(int(ch_min), int(ch_max)))
        ...
        psds_mea=psds_raw_mea-psds_noi
        psds_sig=psds_raw_sig
        ...
        psds_mea   =psds_mea   [inputs_xspec.i_f_min:inputs_xspec.i_f_max]
        psds_sig   =psds_sig   [inputs_xspec.i_f_min:inputs_xspec.i_f_max]
        ...
        for ch_rebin_min, ch_rebin_max, psd_mea, psd_sig, df_rebin\
        in zip(chs_rebin_min, chs_rebin_max, psds_mea, psds_sig, dfs_rebin):
            rates[ch_rebin_min]=psd_mea*df_rebin
            stats_err[ch_rebin_min]=psd_sig*df_rebin
            syses_err[ch_rebin_min]=inputs_xspec.sys_err_psd
        ...
        name_outpha=inputs_xspec.name_inevt.replace('.evt', '_{0:04}_{1:04}_psd.pha'.format(int(ch_min), int(ch_max)))
