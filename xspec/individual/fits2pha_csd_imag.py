...
def main():
    ...
    name_outpha_def='./ni1200120110_csd_imag.pha'
    ...
    for i_b in is_b:
        ...
        csds_mea=hdus[i_b+2].data['CSDM']
        csds_sig=hdus[i_b+2].data['CSDS']
        csds_im_mea=np.imag(csds_mea)
        csds_im_sig=np.imag(csds_sig)
        ...
        csds_im_mea=csds_im_mea[i_f_min:i_f_max]
        csds_im_sig=csds_im_sig[i_f_min:i_f_max]
        ...
        for ch_rebin_min, ch_rebin_max, csd_im_mea, csd_im_sig, df\
        in zip(chs_rebin_min, chs_rebin_max, csds_im_mea, csds_im_sig, dfs):
            # df is necessary for XSPEC to show fP(f) with 'plot euf'.
            rates[ch_rebin_min]=csd_im_mea*df 
            stats_err[ch_rebin_min]=csd_im_sig*df
