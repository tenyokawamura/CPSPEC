...
def main():
    # ----- Setting ----- #
    name_outpdf=inputs_xspec_plot.name_inevt.replace('.evt', '_csds_imag_xspec.pdf')
    xs_pl=[1.e-3, 2.e2]
    ys_pl=[-5.e-3, 5.e-3]
    ...
    for i_n, (ch_min, ch_max) in enumerate(zip(inputs_xspec_plot.chs_min, inputs_xspec_plot.chs_max)):
        ...
        # ----- XSPEC ----- #
        # ----------------- #
        name_intxt=inputs_xspec_plot.name_inevt.replace('.evt', '_{0:04}_{1:04}_{2:04}_{3:04}_csd_imag_xspec.txt'.format(int(ch_min), int(ch_max), int(inputs_xspec_plot.ch_ref_min), int(inputs_xspec_plot.ch_ref_max)))
        fs, dfs, csds_im, dcsds_im\
