...
def main():
    # ----- Setting ----- #
    name_outpdf=inputs_xspec_plot.name_inevt.replace('.evt', '_csds_phase_xspec.pdf')
    xs_pl=[1.e-3, 2.e2]
    ys_pl=[-0.5, 0.5]
    ...
    for i_n, (ch_min, ch_max) in enumerate(zip(inputs_xspec_plot.chs_min, inputs_xspec_plot.chs_max)):
        ...
        # ----- Original ----- #
        # -------------------- #
        name_infits=inputs_xspec_plot.name_inevt.replace('.evt', '_{0:04}_{1:04}_{2:04}_{3:04}_csdf.fits'.format(int(ch_min), int(ch_max), int(inputs_xspec_plot.ch_ref_min), int(inputs_xspec_plot.ch_ref_max)))
        hdus=fits.open(name_infits)
        fs=hdus[1].data['F']
        dfs=hdus[1].data['DF']
        csds_ph=hdus[1].data['CSDPM']
        dcsds_ph=hdus[1].data['CSDPS']
        ...
        # ----- XSPEC ----- #
        # ----------------- #
        name_intxt=inputs_xspec_plot.name_inevt.replace('.evt', '_{0:04}_{1:04}_{2:04}_{3:04}_csd_phase_xspec.txt'.format(int(ch_min), int(ch_max), int(inputs_xspec_plot.ch_ref_min), int(inputs_xspec_plot.ch_ref_max)))
        fs, dfs, csds_ph, dcsds_ph\
