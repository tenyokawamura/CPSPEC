...
for i_n, (ch_min, ch_max) in enumerate(zip(inputs_xspec_plot.chs_min, inputs_xspec_plot.chs_max)):
    name_inpha =inputs_xspec_plot.name_inevt.replace('.evt', '_{0:04}_{1:04}_{2:04}_{3:04}_csd_phase.pha'.format(int(ch_min), int(ch_max), int(inputs_xspec_plot.ch_ref_min), int(inputs_xspec_plot.ch_ref_max)))
    name_outtxt=inputs_xspec_plot.name_inevt.replace('.evt', '_{0:04}_{1:04}_{2:04}_{3:04}_csd_phase_xspec.txt'.format(int(ch_min), int(ch_max), int(inputs_xspec_plot.ch_ref_min), int(inputs_xspec_plot.ch_ref_max)))
    ...
    with open(name_outtxt, 'w') as fout:
        str_header='#f[Hz] df[Hz] \phi(f) d\phi(f)\n'
